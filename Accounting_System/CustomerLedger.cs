            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(DataAccessLayer.Con()))
                {
                    ...
                    string query1 = @"
                        ...
                        FROM LedgerBook 
                        WHERE  PartyID = @d3  AND Date Between  @d1 AND @d2
                        ORDER BY Date, LedgerNo ASC";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(DataAccessLayer.Con()))
                {
                    ...
                    string query1 = @"SELECT Date AS InvoiceDate, Name, LedgerNo, Label, Credit, Debit, Manual_Inv,
                            total_sale  AS total_sale  FROM LedgerBook WHERE PartyID = @d3 ORDER BY Date, LedgerNo ASC";
                    using (SqlCommand cmd = new SqlCommand(query1, con))
                    {
                        cmd.Parameters.Add("@d1", SqlDbType.DateTime).Value = dtpDateFrom.Value.Date;
                        cmd.Parameters.Add("@d2", SqlDbType.DateTime).Value = dtpDateTo.Value.Date.AddDays(1);
