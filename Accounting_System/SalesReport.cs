...
namespace Accounting_System
{
    public partial class SalesReport : Form
    {
            ...
            using (con = new SqlConnection(DataAccessLayer.Con()))
            {
                ...
                string query = @"
                    SELECT 
                        ...
                        InvoiceInfo.Balance,
                        InvoiceInfo.Remarks,
                        Customer.Name, InvoiceInfo.total_sale
                    FROM 
                        InvoiceInfo 
                    INNER JOIN 
                ...
                using (SqlConnection con = new SqlConnection(DataAccessLayer.Con()))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(ctn, con))
                    {
                        ...
                        using (SqlConnection myConnection = new SqlConnection(DataAccessLayer.Con()))
                        {
                           ...
                           FROM Customer INNER JOIN InvoiceInfo ON Customer.ID = InvoiceInfo.CustomerID INNER JOIN Invoice_Product ON InvoiceInfo.Inv_ID = Invoice_Product.InvoiceID INNER JOIN Product ON Invoice_Product.ProductID = Product.PID where InvoiceDate between @d1 and @d2  order by InvoiceDate";
        ...
        private void txtSupplierName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(DataAccessLayer.Con()))
                {
                    ...
                    {
                        cmd.CommandText = "SELECT RTRIM(ID), RTRIM(CustomerID) FROM Customer WHERE Name=@d1";
                        ...
                        using (SqlDataReader rdr = cmd.ExecuteReader())
                        {
                            if (rdr.Read())
                            {
                                ...
                                txtSup_Code.Text = rdr.GetString(1) ?? string.Empty;
                            }
                        }
                    }
                }
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("الرجاء اختيار عميل", "", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            using (con = new SqlConnection(DataAccessLayer.Con()))
            {
                ...
                string query = @"
                    ...
                    INNER JOIN 
                        Customer ON InvoiceInfo.CustomerID = Customer.ID 
                    WHERE 
                        InvoiceInfo.InvoiceDate >= @d3 AND InvoiceInfo.InvoiceDate < @d4 AND Customer.ID=@d5
                    ORDER BY 
                        InvoiceInfo.InvoiceDate";
                System.Data.DataTable dtable = new System.Data.DataTable();
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.Add("@d3", SqlDbType.DateTime).Value = dateTimePicker2.Value.Date;
                    cmd.Parameters.Add("@d4", SqlDbType.DateTime).Value = dateTimePicker1.Value.Date.AddDays(1);
                    cmd.Parameters.Add("@d5", txtSup_ID.Text);
                ...
                using (SqlConnection con = new SqlConnection(DataAccessLayer.Con()))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(ctn, con))
                    {
                        ...
                        using (SqlConnection myConnection = new SqlConnection(DataAccessLayer.Con()))
                        {
                            ...
                            MyCommand1.Connection = myConnection;
                            MyCommand.CommandText = @"SELECT Customer.ID, Customer.Name, Customer.Gender, Customer.Address, Customer.City, Customer.State, Customer.ZipCode, Customer.ContactNo, Customer.EmailID, Customer.Remarks, Customer.Photo, InvoiceInfo.Inv_ID, InvoiceInfo.InvoiceNo, InvoiceInfo.InvoiceDate, InvoiceInfo.CustomerID , InvoiceInfo.GrandTotal, InvoiceInfo.TotalPaid, InvoiceInfo.Balance, Invoice_Product.IPo_ID, Invoice_Product.InvoiceID, Invoice_Product.ProductID, Invoice_Product.CostPrice, Invoice_Product.SellingPrice, Invoice_Product.Margin, Invoice_Product.Qty, Invoice_Product.Amount, Invoice_Product.DiscountPer, Invoice_Product.Discount, Invoice_Product.VATPer, Invoice_Product.VAT, Invoice_Product.TotalAmount, Product.PID, Product.ProductCode, Product.ProductName, InvoiceInfo.total_sale  
                           FROM Customer INNER JOIN InvoiceInfo ON Customer.ID = InvoiceInfo.CustomerID INNER JOIN Invoice_Product ON InvoiceInfo.Inv_ID = Invoice_Product.InvoiceID INNER JOIN Product ON Invoice_Product.ProductID = Product.PID where InvoiceDate between @d1 and @d2 AND Customer.ID=@d5 order by InvoiceDate";
                            MyCommand.Parameters.Add("@d1", SqlDbType.DateTime).Value = dateTimePicker2.Value.Date;
                            MyCommand.Parameters.Add("@d2", SqlDbType.DateTime).Value = dateTimePicker1.Value.Date;
                            MyCommand.Parameters.Add("@d5", txtSup_ID.Text);
                        ...
                        using (SqlConnection con2 = new SqlConnection(DataAccessLayer.Con()))
                        {
                            ...
                            using (SqlCommand cmd2 = new SqlCommand(ct, con2))
                            {
                                cmd2.Parameters.Add("@d1", SqlDbType.DateTime).Value = dateTimePicker2.Value.Date;
                                cmd2.Parameters.Add("@d2", SqlDbType.DateTime).Value = dateTimePicker1.Value.Date;
                                SqlDataReader rdr2 = cmd2.ExecuteReader();
                                if (rdr2.Read())
                                {
                                    a = Convert.ToDecimal(rdr2.GetValue(0));
                        ...
                        using (SqlConnection con3 = new SqlConnection(DataAccessLayer.Con()))
                        {
                            ...
                            using (SqlCommand cmd3 = new SqlCommand(ct1, con3))
                            {
                                cmd3.Parameters.Add("@d1", SqlDbType.DateTime).Value = dateTimePicker2.Value.Date;
                                cmd3.Parameters.Add("@d2", SqlDbType.DateTime).Value = dateTimePicker1.Value.Date;
                                SqlDataReader rdr3 = cmd3.ExecuteReader();
                                if (rdr3.Read())
                                {
                                    d = Convert.ToDecimal(rdr3.GetValue(0));
